...
function HomeContent() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const storedPrompt = sessionStorage.getItem('pending_geometry_prompt');
        if (storedPrompt) {
          setPrompt(storedPrompt);
          sessionStorage.removeItem('pending_geometry_prompt');
        }
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-cyan-500 selection:text-white transition-colors duration-200">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <div ref={userDropdownRef} className="relative">
                {(() => {
                  ...
                  const monthlyAllowance = Number((currentUser as any).monthly_allowance ?? (currentUser as any).monthlyAllowance ?? 0);
                  ...
                  const isUnlimitedCreditsFlag = Boolean(
                    (currentUser as any).is_unlimited ||
                    (currentUser as any).isUnlimited ||
                  ...
                  // 1. Chuẩn hóa điều kiện kiểm tra tài khoản Dùng thử (isTrial):
                  const isTrial = 
                    !isAdmin && 
                    ...
                    monthlyAllowance === 0 && 
                    Boolean(
                      (currentUser as any).is_trial || 
                      (currentUser as any).isTrial || 
                      (lifetimeCredits > 0 && !(currentUser as any).has_paid && !(currentUser as any).hasPaid && !planExp)
                    );

                  const hasUnlimitedTime = isAdmin || (!isFreeAccount && !isTrial && isUnlimitedCreditsFlag && !planExp);
                  ...
                  const isVipFlag = Boolean((currentUser.isVip || (currentUser as any).is_vip) && !(currentUser as any).is_trial && !(currentUser as any).isTrial);
                ...
                {isUserDropdownOpen && (() => {
                  ...
                  const monthlyAllowance = Number((currentUser as any).monthly_allowance ?? (currentUser as any).monthlyAllowance ?? 0);
                  const planExp = (currentUser as any).plan_expires_at || (currentUser as any).planExpiresAt || (currentUser as any).subscription_expires_at || (currentUser as any).subscriptionExpiresAt;
                  const resetAt = (currentUser as any).next_credit_reset_at || (currentUser as any).nextCreditResetAt;
                  const isPlanActive = Boolean(planExp && new Date(planExp) > new Date());
                  const lifetimeCredits = Number((currentUser as any).lifetime_credits ?? (currentUser as any).lifetimeCredits ?? (currentUser as any).lifetime_quota ?? (currentUser as any).lifetimeQuota ?? 0);
                  const hasDualWallet = (currentUser as any).monthly_credits !== undefined || (currentUser as any).lifetime_credits !== undefined || (currentUser as any).subscription_quota !== undefined || (currentUser as any).lifetime_quota !== undefined;
                  ...
                  const badgeBase = 'text-[11px] font-medium leading-none px-2 py-1 rounded-md tracking-normal inline-flex items-center justify-center shrink-0';
                  ...
                  return (
                    <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                      ...
                      <div className="px-3.5 pb-2.5 border-b border-slate-100 dark:border-slate-800/80">
                        <div className="flex items-center gap-2.5">
                          <div className="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 p-0.5 border border-slate-200 dark:border-slate-700 flex-shrink-0 overflow-hidden shadow-xs">
                            <img
                              src={(currentUser as any).avatar || '/avatars/avatar-1.svg'}
                              alt="Avatar"
                      ...
                      <div className={`mx-2.5 my-2 px-3 py-2.5 rounded-xl flex items-center justify-between text-xs ${
                        isFreeAccount
                        ...
                        <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                          {isFreeAccount ? 'Tài Khoản Miễn Phí (Gói Free):' : isTrial ? 'Hạn mức dùng thử:' : 'Tổng hạn mức khả dụng:'}
                        </span>
                      ...
                      {/* Khối Thẻ Hạn Mức (LUÔN HIỆN CẢ 2 VÍ ĐỘC LẬP) */}
                      <div className="mx-2.5 mb-2 flex flex-col gap-2">
                        {/* CARD 1: GÓI THUÊ BAO (Monthly / Pass) */}
                      ...
                      {/* Danh sách nút hành động bên dưới (Menu Items) */}
                      <div className="px-1.5 pt-1 flex flex-col gap-0.5">
                        ...
                        >
                          ...
                          <span className={`${badgeBase} bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-normal border border-slate-200/70 dark:border-slate-700`}>
                        ...
                        {(() => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className={`${badgeBase} bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 font-semibold border border-amber-200/80 dark:border-amber-800/60`}>
                        ...
                        <button
                          ...
                          className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-slate-100/80 dark:hover:bg-slate-800/60 transition-colors cursor-pointer text-left"
                        >
                          <div className="flex items-center gap-2.5">
      ...
      <main className="flex-1 min-h-0 w-full overflow-hidden flex flex-col">
        {/* Vẽ hình học AI (2 Cột Studio) */}
        <div
          suppressHydrationWarning
          className="flex-1 min-h-0 w-full px-4 md:px-6 py-3 flex flex-col lg:flex-row gap-4 overflow-hidden"
        >
