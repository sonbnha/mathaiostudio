...
export default function LessonPlanView({ licenseKey: parentKey = '' }: LessonPlanViewProps) {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    if (licenseInfo.isFullyExpired || licenseInfo.turnsLeft <= 0 || licenseInfo.isExpiredOrDepleted) {
      openRenewModal({
        isNearExpiry: false,
        customTitle: 'Tài khoản đã hết Ω hoặc hết hạn VIP',
        customDescription: 'Vui lòng nạp thêm Ω hoặc kích hoạt License Key để tiếp tục soạn giáo án theo chuẩn Công văn 5512.',
      });
      return;
    }
