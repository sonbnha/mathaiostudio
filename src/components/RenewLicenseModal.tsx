...
export default function RenewLicenseModal({
  ...
  const title = customTitle || (
    isNearExpiry
      ...
      : 'Tài khoản đã hết Ω sử dụng hoặc hết hạn VIP'
  );

  const description = customDescription || (
    isNearExpiry
      ? 'Gói bản quyền của bạn sắp hết hạn hoặc sắp hết Ω. Vui lòng nhập License Key mới để gia hạn và cộng dồn thời gian sử dụng liên tục không gián đoạn.'
      : 'Vui lòng nạp thêm Ω hoặc kích hoạt License Key để tiếp tục tạo hình minh họa toán học và soạn giáo án.'
  );
    ...
    try {
      if (currentUser) {
        ...
        if (!res.ok || !data.valid) {
          throw new Error(data.message || 'Mã License Key không hợp lệ hoặc đã hết Ω.');
        }
